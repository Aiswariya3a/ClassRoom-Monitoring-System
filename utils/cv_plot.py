...
def build_camera_box(rear_size=90):
    ...
    point_3d.append((front_size, -front_size, front_depth))
    point_3d.append((-front_size, -front_size, front_depth))
# In the build_camera_box function, replace np.float with float or np.float64.
    point_3d = np.array(point_3d, dtype=float).reshape(-1, 3)  # Use `float` or `np.float64` instead of `np.float`
